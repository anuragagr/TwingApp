...
@property (nonatomic, strong) CameraFolderButton *cameraFolder;
@property (nonatomic, strong) UIView *topBarView;
@property (nonatomic, strong) UIView *bottomBarView;
@property  (nonatomic, strong) CameraFolderView *cameraFolderView;
...
//    [[NSNotificationCenter defaultCenter] addObserver:self  selector:@selector(orientationChanged:)    name:UIDeviceOrientationDidChangeNotification  object:nil];
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
    {
        ...
        topBarSize = CGSizeMake(self.frame.size.width, self.frame.size.height * 0.10);
        bottomBarSize= CGSizeMake(self.frame.size.width, self.frame.size.height * 0.42);
...
//    }
    ...
    if (_topBarView) {
        ...
        _topBarView.backgroundColor = [UIColor colorWithRed:0 green: 0 blue: 0 alpha: 1];
...
//        }
        ...
        if (_cameraDismiss) {
            _cameraDismiss.frame = (CGRect){0,0,100,30};
            _cameraDismiss.center = CGPointMake(40, _topBarView.center.y+15);
    ...
    if (_bottomBarView) {
        //Setup visual attribution for bar
        _bottomBarView.frame  = (CGRect){0,self.frame.size.height*0.70, bottomBarSize};
       // _bottomBarView.autoresizingMask=UIViewAutoresizingFlexibleBottomMargin || uiv;
        _bottomBarView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha: 1.0];
        ...
        if (_captureManager) {
            ...
            //Button Visual attribution
            _cameraShutter.frame = (CGRect){0,0, 80,80};
            _cameraShutter.center = CGPointMake(self.frame.size.width/2, self.frame.size.height*0.860);
            _cameraShutter.tag = ShutterButtonTag;
            _cameraShutter.backgroundColor = [UIColor clearColor];
            [_cameraShutter setBackgroundImage:[UIImage imageNamed:@"okay.png" ] forState:UIControlStateNormal];
            //Button target
            [_cameraShutter addTarget:self action:@selector(inputManager:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:_cameraShutter];
        }
        ...
        _cameraFlash = [CameraFlashButton new];
        if (_cameraFlash) {
            _cameraFlash.frame = (CGRect){0,0,30,30};
            _cameraFlash.center = CGPointMake(self.frame.size.width/2, self.frame.size.height*0.960);
            _cameraFlash.tag = FlashButtonTag;
            _cameraFlash.backgroundColor=[UIColor clearColor];
            [_cameraFlash setBackgroundImage:[UIImage imageNamed:@"flash.png" ] forState:UIControlStateNormal];
            //Button target
            [_cameraFlash addTarget:self action:@selector(inputManager:) forControlEvents:UIControlEventTouchUpInside];
            if ( UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad ) [self addSubview:_cameraFlash];
        }
        //Add the camera toggle button
        _cameraToggle = [CameraToggleButton new];
        if (_cameraToggle) {
            _cameraToggle.frame = (CGRect){0,0, 50,50};
            _cameraToggle.center = CGPointMake(self.frame.size.width/2-100, self.frame.size.height*0.860);
            _cameraToggle.tag = ToggleButtonTag;
            _cameraToggle.backgroundColor=[UIColor clearColor];
             [_cameraToggle setBackgroundImage:[UIImage imageNamed:@"camera.png" ] forState:UIControlStateNormal];
            //Button target
        ...
        //Add the camera folder button
        _cameraFolder = [CameraFolderButton new];
        if (_cameraFolder) {
            _cameraFolder.frame = (CGRect){0,0,50,50};
            _cameraFolder.center = CGPointMake(self.frame.size.width/2+100, self.frame.size.height*0.860);
            _cameraFolder.tag = FolderButtonTag;
            _cameraFolder.backgroundColor=[UIColor clearColor];
            [_cameraFolder setBackgroundImage:[UIImage imageNamed:@"folder.png" ] forState:UIControlStateNormal];
            //Button target
            [_cameraFolder addTarget:self action:@selector(inputManager:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:_cameraFolder];
        }
...
//    singleTapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(focusGesture:)];
...
-(void)inputManager:(id)sender {
    ...
    switch ([(UIButton *)sender tag]) {
        ...
        case FolderButtonTag:   [self onTapFolderButton];    return;
    }
}
...
- (void)onTapFolderButton
{
    //[self setNeedsStatusBarAppearanceUpdate];
   // singleTapGestureRecognizer.cancelsTouchesInView=NO;
    //Instantiate the camera view & assign its frame
    _cameraFolderView = [[CameraFolderView alloc] initWithFrame:self.frame];
   ...
   // _cameraView.delegate = self;
    ...
    [applicationLoadViewIn setType:kCATransitionReveal];
    [applicationLoadViewIn setTimingFunction:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn]];
    [[_cameraFolderView layer]addAnimation:applicationLoadViewIn forKey:kCATransitionReveal];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveTestNotification:)
                                                 name:@"TestNotification"
                                               object:nil];
    [self addSubview:_cameraFolderView];
